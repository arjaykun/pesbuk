...
@php
	$text = $reply->likes_count . ' ' . Str::of('Like')->plural($reply->likes_count); 
@endphp
<div>
	@if (!$reply->liked())
		<x-forms.icon-button text="{{ $text }}" :bordered="false" wire:click="likeReply({{ $reply }})" wire:loading.attr="disabled" >
	...
	@else
		<x-forms.icon-button text="{{ $text }}" :bordered="false" wire:click="unlikeReply({{ $reply }})" wire:loading.attr="disabled" >
