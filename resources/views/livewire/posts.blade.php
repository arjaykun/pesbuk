    ...
    <div :class="{'relative':showOption}">
      ...
      >
        ...
        @can('modify-post', $post)
          ...
          {{-- remove button --}}
          <x-forms.icon-button text="Remove" :bordered='false' class="text-xs" @click="document.getElementById('confirmModal').classList.toggle('hidden');document.querySelector('#deleteForm').action='{{  route('posts.destroy', ['post' => $post]) }}'">
            <svg class="w-3 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
